...
//                       ];
    ...
    UIImage *flipCameraImage = [UIImage imageNamed:@"switchCamera"];
    
    
    id flipCamera = [[[UIBarButtonItem alloc] autorelease]
                     initWithImage:flipCameraImage
                     //style:UIBarButtonItemStylePlain
...
- (UIImage*)buildReticleImage {
    ...
    if (self.processor.is1D) {
        UIColor* color = [UIColor colorWithRed:1.0 green:0.0 blue:0.0 alpha:0.0];
    ...
    if (self.processor.is2D) {
        UIColor* color = [UIColor colorWithRed:0.0 green:1.0 blue:0.0 alpha:0.0];
        CGContextSetStrokeColorWithColor(context, color.CGColor);
        CGContextSetLineWidth(context, RETICLE_WIDTH);
